...
def cache_checkout_data(request):
    """
    Function to cache the shopping basket,
    order and user data if it catches an error
    and doesn't go through. Credit: Boutique Ado project, Code Institute.
    """
    try:
...
def checkout(request):
    """
    Function to place an order using the basket contents, user
...
def checkout_success(request, order_number):
    ...
    if request.user.is_authenticated:
        ...
        if save_info:
            ...
            user_account_form = UserAccountForm(account_data, instance=profile)
